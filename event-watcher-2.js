...
const handleTokenDepositEvent = async (event, provider, contract) => {
    ...
    try {
        ...
        console.log(
            `confirmed with :>> ${txReceipt.confirmations} confirmations`
        )
    } catch (err) {
...
const main = async () => {
    ...
    const hyperspaceNetworkId =
        await hyperspaceWebSockerProvider.eth.net.getId() //I have the Id in the event
